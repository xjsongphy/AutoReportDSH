...
- **Task and artifact tracking** — `send_to_agent` records durable tasks and revisions; specialists finish with `report_workflow`; manifests live under `$DSH_HOME`, not in the experiment folder
...
- **Small model surface** — MAIN adds `send_to_agent` (plus DSH `ask_user_question`); specialists add `report_workflow`; everything else is DSH `read` / `write` / `edit` / `bash` / `skill`
...
- **Python environments** — AutoReport-managed venv (`$DSH_HOME/autoreport/venv`), a detected local interpreter, or a custom path; owned bash sees `DSH_AUTOREPORT_PYTHON` and a PATH prefix so bare `python3` hits that interpreter
...
- **Python** — managed (`__managed__` → `$DSH_HOME/autoreport/venv`, created on save with `uv venv --seed` or `python3 -m venv`), local (conda / virtualenv / pyenv / PATH, including `~/.autoreport/venv` if present), or a custom absolute path
