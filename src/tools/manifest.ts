...
export function installManifestTool(ctx: Context, hostCtx: Context, role: AutoReportRole): () => void {
  ...
  try {
    disposeTool = ctx.tools.register(defineTool({
      name: 'manifest',
      description: [
        'AutoReport semantic manifest for agent-produced file handoff; this is not a full workspace index. It tracks successful file mutations made by each role, while agents maintain semantic descriptions and role-level notes. Use list_directory when available for directory discovery and pre-existing file inventory.',
